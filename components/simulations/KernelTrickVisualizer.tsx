...
export default function KernelTrickVisualizer() {
  ...
  const initPlot = () => {
    ...
    // Data Gen
    const N_inner = 100, N_outer = 150;
    let x: number[] = [], y: number[] = [], z_flat: number[] = [], z_lifted: number[] = [], colors: string[] = [];
    ...
    for (let i = 0; i < N_inner; i++) {
        ...
        x.push(xi); y.push(yi); z_flat.push(0); z_lifted.push((xi*xi + yi*yi) * 0.8);
        colors.push('#ff385f');
    }
    // Outer Ring
    ...
    tracesRef.current = {
        x, y, z_flat, z_lifted, colors, plane_x, plane_y, plane_z
    };
    ...
    const scatterTrace = {
        ...
        mode: 'markers',
        type: 'scatter3d',
        marker: { size: 5, color: colors, opacity: 0.8, line: {color: 'white', width: 0.5} },
        name: 'Data Points'
    };
    ...
    layoutRef.current = {
        ...
        scene: {
            ...
            zaxis: {title: 'Kernel Dimension', range: [0, 35], showgrid: true, zeroline: false},
            aspectmode: 'manual', 
  ...
  const updateView = (view: "2d" | "3d" | "plane") => {
      ...
      window.Plotly.react(plotDivRef.current, 
        [
            {...tracesRef.current.scatterTrace, z: updateData.z[0], marker: {...tracesRef.current.scatterTrace?.marker, opacity: updateData['marker.opacity'][0]}}, 
      ...
      window.Plotly.animate(plotDivRef.current, {
          layout: updateLayout
      ...
      const scatter = {
          ...
          mode: 'markers',
          type: 'scatter3d',
          marker: { size: 6, color: t.colors, opacity: view === 'plane' ? 0.3 : 0.8, line: {color: 'white', width: 0.5} },
      };
      ...
      if(updateLayout['scene.camera.eye']) {
        window.Plotly.animate(plotDivRef.current, { layout: updateLayout }, {transition: {duration: 1000}, frame: {duration: 1000}});
      }
  };
